 ...
 * 
 */
#define BUFFER_IDX(x, y) ((y * BYTES_PER_ROW) + (x >> 3))

/**
 * @brief Macro to get number of bits to shift based on x direction
 * 
 */
#define PIXEL_SHIFT_VALUE(x) (x & 0x7)

/**
 * @brief Macro to get mask to set pixel
 * 
 */
#define SET_PIXEL(x) (1 << PIXEL_SHIFT_VALUE(x))

/**
 * @brief Macro to get clear pixel mask
 * 
 */
#define CLEAR_PIXEL(x) (0xFF ^ SET_PIXEL(x))
 ...
 */
bool prv_validate_buffer_descriptor(const struct device *dev, uint16_t x, uint16_t y, const struct display_buffer_descriptor *desc) {
...
static int sim_display_init(const struct device *dev) {
    ...
    err = SDL_CreateWindowAndRenderer(config->width, config->height, 0, &disp_data->window, &disp_data->renderer);
    if (err != 0) {
...
static int sim_display_write(
    ...
    for (uint16_t _y = y; _y < (y + desc->pitch); _y++) {
        for (uint16_t _x = _x; _x < config->width; _x++) {
...
static int sim_display_set_brightness(
    const struct device *dev,
...
static int sim_display_set_contrast(const struct device *dev,
                    const uint8_t contrast)
{
    return -ENOTSUP;
}
 
 static void sim_display_get_capabilities(const struct device *dev,
         struct display_capabilities *capabilities)
 {
 }
 
 static int sim_display_set_pixel_format(const struct device *dev,
 ...
 #define SIM_DISPLAY_DEFINE(n)						\
     ...
     static sim_display_data_t sim_display_data_##n;			\
                                     \
     DEVICE_DT_INST_DEFINE(n, &sim_display_init, NULL,		\
                   &sim_display_data_##n,				\
                   &sim_display_config_##n,				\
                   POST_KERNEL,				\
