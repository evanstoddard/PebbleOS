...
typedef struct ble_client_t {
  ...
  /* Called when all handles should be invalidated (e.g., disconnect,
   * re-discovery) */
  ...
  /* Called on GATT notification/indication for a characteristic this client
   * owns */
  ...
  void (*on_read)(uint16_t handle, const uint8_t *data, uint16_t len,
                  uint8_t err);
