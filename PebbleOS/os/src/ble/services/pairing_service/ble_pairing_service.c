...
#define BT_UUID_PPS_CONN_STATUS BT_UUID_DECLARE_128(PEBBLE_BASE_UUID_BYTES, 0x01, 0x00, 0x00, 0x00)
...
#define BT_UUID_PPS_CONN_PARAMS BT_UUID_DECLARE_128(PEBBLE_BASE_UUID_BYTES, 0x05, 0x00, 0x00, 0x00)
...
BT_GATT_SERVICE_DEFINE(
    prv_pebble_pairing_svc, BT_GATT_PRIMARY_SERVICE(BT_UUID_PEBBLE_PAIRING_SERVICE),

    /* Connection Status characteristic */
    BT_GATT_CHARACTERISTIC(BT_UUID_PPS_CONN_STATUS, BT_GATT_CHRC_READ | BT_GATT_CHRC_NOTIFY,
                           BT_GATT_PERM_READ, prv_on_read_conn_status, NULL, NULL),
    BT_GATT_CCC(prv_on_conn_status_ccc_changed, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE),

    /* Trigger Pairing characteristic */
    BT_GATT_CHARACTERISTIC(BT_UUID_PPS_TRIGGER_PAIRING, BT_GATT_CHRC_READ | BT_GATT_CHRC_WRITE,
 ...
 */
static void prv_populate_conn_status_struct(struct bt_conn *conn, PPSConnectivityStatus_t *status) {
...
static void prv_on_conn_status_ccc_changed(const struct bt_gatt_attr *attr, uint16_t value) {
  LOG_INF("Connection status characteristic configuration changed: 0x%04X %p", value, attr);
...
static void prv_on_conn_params_ccc_changed(const struct bt_gatt_attr *attr, uint16_t value) {
  LOG_INF("Connection params characteristic configuration changed: 0x%04X", value);
}
...
int ble_pairing_service_init(void) {
  ...
  static struct bt_conn_auth_info_cb auth_cb = {.pairing_complete = prv_on_pairing_complete};
