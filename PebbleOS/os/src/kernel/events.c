 ...
 * @brief Inter-thread event dispatch system. Maintains separate message queues
 * for events originating from ISRs, the kernel background thread, the kernel
 * main thread, the app thread, and the worker thread. Events are routed to the
 * appropriate queue based on the calling context, and consumers dequeue via
 * events_take() which polls all queues simultaneously.
 */
