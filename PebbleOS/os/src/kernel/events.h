 ...
 * @brief PebbleOS event type definitions and inter-thread event queue API.
 * Defines all system event IDs and the structures used to carry event payloads
 * between threads.
 */
...
typedef enum {
  ...
  //! Event sent back to the app to let them know the result of their sent
  //! message.
  PEBBLE_APP_OUTBOX_SENT_EVENT,
  //! A request from the app to the outbox service to handle a message.
  //! @note The consuming service must call app_outbox_service_consume_message()
  //! to clean up. In case the event is dropped because the queue is reset,
  //! cleanup happens by events.c in event_queue_cleanup_and_reset().
 ...
 * @brief Payload for a PEBBLE_CALLBACK_EVENT. Carries a function pointer and
 * an opaque data argument to be invoked on the target thread's event loop.
 *
 */
...
} __attribute__((__packed__)) PebbleEvent_t;
...
/**
 * @brief Put event onto proper queue
 *
 * @param event Pointer to event
 * @return Return status of queuing event
 * @retval 0 Success
...
/**
 * @brief Attempt to take event from one of the event queues
 *
 * @param event Pointer to write dequeued event to
 * @param timeout_ms Amount of time to block waiting for event in milliseconds
 * @return Indicates whether event was taken or if there are any errors
 * @retval -EAGAIN No event taken
 * @retval -EINVAL Bad input arguments
 * @retval 0 Event taken
 */
